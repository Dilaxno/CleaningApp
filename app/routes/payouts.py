...
class PayoutRequest(BaseModel):
    ...
    payout_details: Optional[dict] = (
        None  # e.g., {"bank_name": "...", "account_holder": "...", "account_number": "...", "routing_number": "..."} or {"paypal_email": "..."}
    )
...
async def get_payment_summary(
    ...
    total_pending = (
        db.query(func.sum(Invoice.total_amount))
        .filter(Invoice.user_id == current_user.id, Invoice.status.in_(["pending", "sent"]))
        .scalar()
        or 0
    )
    ...
    total_paid_out = (
        ...
        .filter(Payout.user_id == current_user.id, Payout.status == "completed")
        .scalar()
        or 0
    )
...
async def get_client_payments(
    ...
    for client in clients:
        ...
        total_pending = (
            db.query(func.sum(Invoice.total_amount))
            .filter(Invoice.client_id == client.id, Invoice.status.in_(["pending", "sent"]))
            .scalar()
            or 0
        )
...
async def request_payout(
    ...
    if len(invoices) != len(data.invoice_ids):
        raise HTTPException(
            status_code=400, detail="Some invoices are not found, not paid, or don't belong to you"
        )

    # Check if any invoice is already in a pending payout
    existing_payouts = (
        db.query(Payout)
        .filter(Payout.user_id == current_user.id, Payout.status.in_(["pending", "processing"]))
        .all()
    )
...
async def get_available_invoices_for_payout(
    ...
    query = db.query(Invoice).filter(Invoice.user_id == current_user.id, Invoice.status == "paid")
...
async def get_last_payout_method(
    ...
    return {"payout_method": last.payout_method, "payout_details": last.payout_details or None}
