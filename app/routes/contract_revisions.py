...
async def request_contract_revision(
    ...
    if contract.provider_signature:
        raise HTTPException(
            status_code=400, 
            detail="Cannot request revision after provider has signed"
        )
    ...
    if revision.revision_type not in ['pricing', 'scope', 'both']:
        raise HTTPException(
            status_code=400,
            detail="revision_type must be 'pricing', 'scope', or 'both'"
        )
    ...
    if revision.revision_type in ['pricing', 'both']:
        if not revision.custom_quote:
            raise HTTPException(
                status_code=400,
                detail="custom_quote required when requesting pricing changes"
            )
    ...
    if revision.revision_type in ['scope', 'both']:
        if not revision.custom_scope:
            raise HTTPException(
                status_code=400,
                detail="custom_scope required when requesting scope changes"
            )
...
async def approve_contract_revision(
    ...
    if not contract.revision_requested:
        raise HTTPException(
            status_code=400,
            detail="No revision pending for this contract"
        )
