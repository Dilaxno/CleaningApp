        ...
        try:
            ...
            try:
                ...
                job_status = await asyncio.wait_for(
                    job.status(), 
                    timeout=15.0  # 15 second timeout for status check
                )
                ...
                if job_status == JobStatus.complete:
                    try:
                        job_result = await asyncio.wait_for(
                            job.result(), 
                            timeout=10.0  # 10 second timeout for result
                        )
            ...
            if attempt == max_retries - 1:
                raise HTTPException(
                    status_code=504, 
                    detail="Timeout connecting to job queue - please try again"
                )
        except Exception as e:
            logger.warning(f"🔄 Retry {attempt + 1}/{max_retries} for job {job_id}: {str(e)}")
            if attempt == max_retries - 1:
                logger.error(f"❌ All retries failed for job {job_id}: {str(e)}")
                raise HTTPException(status_code=500, detail="Failed to retrieve job status after retries")
