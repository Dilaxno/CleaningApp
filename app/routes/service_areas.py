...
rate_limit_validation = create_rate_limiter(
    limit=20, window_seconds=60, key_prefix="zipcode_validation"
)
...
async def get_service_areas(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve service areas") from e
...
async def update_service_areas(
    ...
    try:
        ...
        success = validator.update_service_areas_for_business(
            current_user.firebase_uid, service_areas_data
        )
        ...
        raise HTTPException(status_code=500, detail="Failed to update service areas") from e
...
async def validate_zipcode(
    ...
    try:
        ...
        is_valid, message = validator.validate_zipcode_for_business(
            request.zipcode, request.business_uid
        )
        ...
        logger.error(
            f"Error validating ZIP code {request.zipcode} for business {request.business_uid}: {e}"
        )
        raise HTTPException(status_code=500, detail="Failed to validate ZIP code") from e
