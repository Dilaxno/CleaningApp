...
from ..models import User, IntegrationRequest, IntegrationRequestVote
...
async def get_submit_status(
    ...
    recent_request = db.query(IntegrationRequest).filter(
        and_(
            IntegrationRequest.user_id == current_user.id,
            IntegrationRequest.created_at >= one_month_ago
        )
    ).order_by(IntegrationRequest.created_at.desc()).first()
...
async def get_integration_requests(
    ...
    requests = db.query(IntegrationRequest).order_by(
        IntegrationRequest.upvotes.desc(),
        IntegrationRequest.created_at.desc()
    ).all()
    
    # Get user's votes
    user_votes = db.query(IntegrationRequestVote.integration_request_id).filter(
    ...
    for req in requests:
        ...
        result.append(IntegrationRequestResponse(
            id=req.id,
            name=req.name,
            ...
            submitted_by=submitter.full_name or submitter.email.split('@')[0] if submitter else "Anonymous",
...
async def create_integration_request(
    ...
    existing = db.query(IntegrationRequest).filter(
        func.lower(IntegrationRequest.name) == func.lower(data.name)
    ).first()
...
async def upvote_integration_request(
    ...
    existing_vote = db.query(IntegrationRequestVote).filter(
        and_(
