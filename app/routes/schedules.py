                ...
                if client and client.email and contract.square_invoice_url:
                    try:
                        ...
                        await send_invoice_payment_link_email(
                            ...
                            business_name=current_user.full_name or current_user.business_name or "Your Service Provider",
                        ...
                        logger.error(f"❌ Failed to send Square invoice email to {client.email}: {str(e)}")
                        logger.exception(e)
                        # Don't fail the schedule approval, but log the error prominently
                        logger.error(f"⚠️ IMPORTANT: Client {client.email} did not receive invoice email. Use /contracts/{contract.id}/send-square-invoice-email to resend.")
                else:
                    if not client:
