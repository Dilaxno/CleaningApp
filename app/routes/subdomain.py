...
def check_dns_record(record_type: str, name: str, expected_value: str, priority: Optional[int] = None) -> bool:
    """
    ...
    try:
        if record_type == "CNAME":
            ...
            for answer in answers:
                txt_value = ''.join([part.decode() if isinstance(part, bytes) else str(part) for part in answer.strings])
...
async def verify_subdomain(
    ...
    all_verified, updated_records = verify_subdomain_dns(
        config.email_subdomain,
        config.subdomain_dns_records
    )
    ...
    if all_verified:
        ...
        message = "Subdomain verified successfully! You can now send emails from your custom domain."
        ...
        logger.warning(f"Subdomain verification failed for user {current_user.id}: {config.email_subdomain}")
...
async def get_subdomain_status(
    ...
    if config.subdomain_verification_status == "pending":
        verified_count = sum(1 for r in (config.subdomain_dns_records or []) if r.get("status") == "verified")
    ...
    return SubdomainStatusResponse(
        ...
        verified_at=config.subdomain_verified_at.isoformat() if config.subdomain_verified_at else None,
        last_check_at=config.subdomain_last_check_at.isoformat() if config.subdomain_last_check_at else None,
