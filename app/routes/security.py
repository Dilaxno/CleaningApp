...
async def get_security_status(
    ...
    return SecurityStatusResponse(
        ...
        recovery_email=current_user.recovery_email if current_user.recovery_email_verified else None,
...
async def send_recovery_email_verification(
    ...
    if request.target == current_user.email:
        raise HTTPException(
            status_code=400,
            detail="Recovery email must be different from primary email"
        )
...
async def view_backup_codes(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve backup codes")
