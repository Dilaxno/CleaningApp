...
from ..models import User, Client, CustomQuoteRequest, Contract, BusinessConfig
...
async def submit_custom_quote_request(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="Failed to create custom quote request")


@router.get("/public/{public_id}")
async def get_custom_quote_by_public_id(
    public_id: str,
    db: Session = Depends(get_db)
):
    """
    ...
    """
    try:
        quote_request = db.query(CustomQuoteRequest).filter(
        ...
        business_config = db.query(BusinessConfig).filter(
            BusinessConfig.user_id == quote_request.user_id
        ).first()
    ...
    try:
        ...
        for req in requests:
            ...
            result.append({
                ...
                "client_name": client.contact_name or client.business_name if client else "Unknown",
...
async def get_custom_quote_request(
    ...
    try:
        quote_request = db.query(CustomQuoteRequest).filter(
            CustomQuoteRequest.id == request_id,
            CustomQuoteRequest.user_id == current_user.id
        ).first()
        ...
        return {
            ...
            } if quote_request.video_r2_key else None,
            "custom_quote_amount": quote_request.custom_quote_amount,
            "custom_quote_description": quote_request.custom_quote_description,
            "custom_quote_line_items": quote_request.custom_quote_line_items,
            "custom_quote_notes": quote_request.custom_quote_notes,
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve request")
...
async def submit_custom_quote(
    ...
    try:
        ...
        quote_request.custom_quote_line_items = [item.dict() for item in quote_data.line_items] if quote_data.line_items else None
...
async def send_custom_quote_request_notification(
    ...
    try:
        business_config = db.query(BusinessConfig).filter(
            BusinessConfig.user_id == provider.id
        ).first()
