...
from ..models import User, CalendlyIntegration
...
async def cancel_calendly_event(
    ...
    try:
        ...
        if e.response.status_code == 403:
            logger.error(f"❌ 403 Forbidden cancelling event {event_uuid} for user {current_user.id}")
            raise HTTPException(
                status_code=403, 
            ...
            logger.error(f"❌ 404 Not Found cancelling event {event_uuid} for user {current_user.id}")
            raise HTTPException(
                status_code=404, 
                detail="Event not found. The consultation may have already been cancelled or the event ID is incorrect."
            )
        elif e.response.status_code == 401:
            logger.error(f"❌ 401 Unauthorized cancelling event {event_uuid} for user {current_user.id}")
            raise HTTPException(
                status_code=401, 
                detail="Authentication failed. Please disconnect and reconnect your Calendly account."
            )
        else:
            logger.error(f"❌ HTTP {e.response.status_code} cancelling event {event_uuid} for user {current_user.id}: {str(e)}")
            raise HTTPException(
                status_code=e.response.status_code, 
                detail=f"Failed to cancel consultation: {str(e)}"
            )
    except Exception as e:
        logger.error(f"❌ Unexpected error cancelling event {event_uuid} for user {current_user.id}: {str(e)}")
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to cancel consultation: {str(e)}"
        )
