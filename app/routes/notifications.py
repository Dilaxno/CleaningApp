...
async def get_payment_notifications(
    ...
    last_check = user.last_payment_check or datetime(
        2020, 1, 1
    )  # Default to old date if never checked

    recent_payments_query = (
        db.query(Invoice)
        .filter(Invoice.user_id == user.id, Invoice.status == "paid", Invoice.paid_at > last_check)
        ...
        recent_payments.append(
            {
                ...
                "client_name": (
                    invoice.client.business_name or invoice.client.contact_name
                    if invoice.client
                    else "Unknown"
                ),
            }
    ...
    return PaymentNotificationResponse(
        unread_count=user.unread_payments_count, recent_payments=recent_payments
    )
...
async def get_notification_preferences(
    ...
    return {
        "notify_new_clients": (
            user.notify_new_clients if hasattr(user, "notify_new_clients") else True
        ),
        "notify_contract_signed": (
            user.notify_contract_signed if hasattr(user, "notify_contract_signed") else True
        ),
        "notify_schedule_confirmed": (
            user.notify_schedule_confirmed if hasattr(user, "notify_schedule_confirmed") else True
        ),
        "notify_payment_received": (
            user.notify_payment_received if hasattr(user, "notify_payment_received") else True
        ),
