...
QUICKBOOKS_REDIRECT_URI = os.getenv("QUICKBOOKS_REDIRECT_URI", f"{FRONTEND_URL}/auth/quickbooks/callback")
...
if QUICKBOOKS_ENVIRONMENT == "production":
    ...
    QUICKBOOKS_DISCOVERY_URL = "https://developer.api.intuit.com/.well-known/openid_sandbox_configuration"
...
async def get_quickbooks_integration(user_id: int, db: Session) -> Optional[QuickBooksIntegration]:
    ...
    return db.query(QuickBooksIntegration).filter(
        QuickBooksIntegration.user_id == user_id
    ).first()
...
async def oauth_callback_handler(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="Failed to complete QuickBooks connection")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client_http:
            ...
            if response.status_code not in [200, 201]:
                ...
                raise HTTPException(status_code=400, detail=f"Failed to sync customer: {error_detail}")
...
async def sync_invoice(
    ...
    try:
        ...
        customer_sync = db.query(QuickBooksSyncLog).filter(
        ...
        ).order_by(QuickBooksSyncLog.created_at.desc()).first()
            ...
            "Line": [
                {
                    ...
                    "SalesItemLineDetail": {
                        "Qty": 1,
                        "UnitPrice": contract.total_value
                    }
                }
            ],
            "DueDate": (contract.start_date + timedelta(days=15)).strftime("%Y-%m-%d") if contract.start_date else None
        }
        ...
        async with httpx.AsyncClient() as client_http:
            ...
            if response.status_code not in [200, 201]:
                ...
                raise HTTPException(status_code=400, detail=f"Failed to sync invoice: {error_detail}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to sync invoice: {str(e)}")
...
async def sync_payment(
    ...
    try:
        ...
        invoice_sync = db.query(QuickBooksSyncLog).filter(
            QuickBooksSyncLog.user_id == current_user.id,
        ...
        payment_data = {
            ...
            "CustomerRef": {"value": invoice_sync.sync_data.get("invoice_data", {}).get("CustomerRef", {}).get("value")},
            "Line": [
                {
                    "Amount": contract.total_value,
                    "LinkedTxn": [
                        {
                            "TxnId": invoice_sync.quickbooks_id,
                            "TxnType": "Invoice"
                        }
                    ]
                }
        ...
        async with httpx.AsyncClient() as client_http:
            ...
            if response.status_code not in [200, 201]:
                ...
                raise HTTPException(status_code=400, detail=f"Failed to sync payment: {error_detail}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to sync payment: {str(e)}")
