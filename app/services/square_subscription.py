...
async def create_square_subscription(
    ...
    try:
        ...
        if not contract.frequency or contract.frequency.lower() in ["one-time", "per-turnover", "on-demand", "as-needed"]:
        ...
        if contract.square_subscription_id:
            logger.info(f"Subscription already exists for contract {contract.id}: {contract.square_subscription_id}")
            return {
                "success": False,
        ...
        if contract.square_payment_status != "paid":
            logger.info(f"First payment not completed for contract {contract.id}, cannot create subscription")
            return {
                "success": False,
        ...
        async with httpx.AsyncClient(timeout=30.0) as http_client:
            ...
            logger.info(f"✅ Square subscription created successfully: {subscription_id} for contract {contract.id}")
