        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 403:
                    logger.error(f"❌ 403 Forbidden when cancelling event {event_uuid}. This could mean:")
                    logger.error("   - The OAuth token doesn't have sufficient permissions")
                    logger.error("   - The user is not the owner of this event")
                    logger.error("   - The event is already cancelled or in a non-cancellable state")
                    raise httpx.HTTPStatusError(
                        f"Permission denied. You may not be the owner of this event or it may already be cancelled.",
                        request=response.request,
                        response=response
                    )
                elif response.status_code == 404:
                    logger.error(f"❌ 404 Not Found when cancelling event {event_uuid}. Event may not exist or UUID is incorrect.")
                    raise httpx.HTTPStatusError(
                        f"Event not found. The event may have already been cancelled or the ID is incorrect.",
                        request=response.request,
                        response=response
                    )
                elif response.status_code == 401:
                    logger.error(f"❌ 401 Unauthorized when cancelling event {event_uuid}. Token may be expired.")
                    raise httpx.HTTPStatusError(
                        f"Authentication failed. Please reconnect your Calendly account.",
                        request=response.request,
                        response=response
                    )
