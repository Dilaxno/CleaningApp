...
class CustomTemplate(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    base_template_id = Column(String(50), nullable=False)  # e.g., "office", "residential"
    name = Column(String(255), nullable=False)  # Custom name for the template
    description = Column(String(1000), nullable=True)  # Custom description
    ...
    #           "order": 1,
    #           "enabled": true,
    #           "locked": false  // Special fields like requirements cannot be reordered
    #         }
    #       ]
    #     }
    #   ]
    ...
    # Metadata
    is_active = Column(Boolean, default=True)  # Whether this template is currently in use
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
