...
def run_migration():
    logger.info(
        "Running migration to add new cleaning templates (outside-cleaning, carpet-cleaning)..."
    )
    ...
    try:
        with engine.connect() as connection:
            ...
            logger.info(
                f"✅ Successfully updated {rows_updated} business configs with new cleaning templates."
            )
            ...
            result_empty = connection.execute(
                sql_empty, {"new_templates": json.dumps(all_templates)}
            )
            connection.commit()
            rows_empty = result_empty.rowcount
            logger.info(
                f"✅ Successfully set default templates for {rows_empty} business configs with empty templates."
            )
